...
// - Event-Driven Architecture: buttons use interrupt callback for fast response
...
//
// Controls:
// - BT2/JOY (Buttons): Available for game use; joystick press is used for menu confirmation
...
// Economy System
#include "Economy.h"
extern uint16_t coins; // Externally reference Game 3's local coin variable
...
// ===== EXTERNAL INTERRUPT HANDLER FOR BUTTON INPUT =====
/**
...
int main(void)
{
    ...
    PWM_SetDuty(&pwm_cfg, 0);   // LED starts dark - intro will fade it up
    ...
    printf("Menu system initialized. Press JOY to select.\n");

    // ===== MAIN GAME LOOP =====
    // Hybrid pattern: Each component (menu/games) runs its own loop
    // and returns the next state when done
    //
    // First boot: play intro animation once, then fall through to HOME.
    // Returning from any game also goes back to HOME (not INTRO again).
    MenuState current_state = MENU_STATE_INTRO;
    
    while (1)
    {
        switch (current_state) {
            case MENU_STATE_INTRO:
                // Play opening animation (5s, JOY to skip).
                // After it finishes, always go to HOME.
                Menu_RunIntro();
            ...
            {
                // ECONOMY INTERCEPTOR: Inject global gold into local variable
                coins = (uint16_t)Get_Total_Gold();

                // Run Game 3 - returns next state when done
                current_state = Game3_Run();

                // ECONOMY INTERCEPTOR: Synchronize changes back to global
                int difference = (int)coins - Get_Total_Gold();
                if (difference > 0) {
                    Add_Gold(difference);    // Game 3 earned money
                } else if (difference < 0) {
                    Spend_Gold(-difference); // Game 3 spent money
                }
                break;
            }
